      ...
      <section style={{ marginTop: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 8 }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input type="checkbox" checked={createRequestOption} onChange={(e) => setCreateRequestOption(e.target.checked)} /> Create request (also add to requests)
          </label>
          <button onClick={handleCheckout} disabled={checkoutLoading || !selectedComponent} style={{ padding: '8px 10px', borderRadius: 8, background: 'var(--accent)', color: 'white', border: 'none' }}>{checkoutLoading ? 'Checking out…' : 'Checkout'}</button>
        </div>
      </section>
    </div>
  );
